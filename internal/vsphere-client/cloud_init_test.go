...
// testPostStartScript provides a basic test of cloud-init hook script functionality.
const testPostStartScript = `#!/bin/bash
SOURCE="${BASH_SOURCE[0]}"
while [ -h "$SOURCE" ]; do # resolve $SOURCE until the file is no longer a symlink
  DIR="$( cd -P "$( dirname "$SOURCE" )" >/dev/null 2>&1 && pwd )"
  SOURCE="$(readlink "$SOURCE")"
...
echo "$(pwd)" > post-start-script.out
echo "$GOVC_VM" >> post-start-script.out
exit 0
`

func setupSuccessfulPostStartScript(t *testing.T) (temporaryDirectory, scriptPath, scriptOutputPath string) {
	temporaryDirectory = t.TempDir()
	scriptPath = filepath.Join(temporaryDirectory, "post-start-script")
	scriptOutputPath = filepath.Join(temporaryDirectory, "post-start-script.out")

	err := os.WriteFile(scriptPath, []byte(testPostStartScript), os.FileMode(0755))
	require.NoError(t, err)
	return
}
